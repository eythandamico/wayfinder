...
/** Default deck — Activity lands first (signals + agent updates the
 *  user wants on app open), but Chart and Trade are now one swipe
 *  away on either side. Trading apps need the chart + ticket to be
 *  thumbs-reachable; burying the chart at index 3 was the previous
 *  ordering's biggest miss. The user can re-curate via the panel
 *  manager sheet. */
const DEFAULT_PANELS: PanelInstance[] = [
  { id: "p-chart", type: "chart" },
  { id: "p-activity", type: "activity" },
  { id: "p-trade", type: "trade" },
  { id: "p-portfolio", type: "portfolio" },
  { id: "p-watchlist", type: "watchlist" },
  { id: "p-orderbook", type: "orderbook" },
];

/** Default landing index = Activity. One swipe left → Chart; one
 *  swipe right → Trade. The two most-used trading surfaces are
 *  always a thumb-flick away from the landing screen. */
...
export function MobileLayout() {
  ...
  // Hamburger drawer + the secondary sheets it launches. Friends &
  // Deposit reuse the desktop's global open-state hooks so any
  // trigger in the app gets the right chrome for the breakpoint;
  // Settings is local because it's a view-mode on desktop and has
  // no global open-state to bridge.
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-background">
      ...
      {/* Drawer — opens via the hamburger in MobileTopBar. Each row
       *  closes this sheet and launches its destination sheet:
       *  Friends + Deposit through global open-state hooks (so they
       *  also work from any other trigger), Settings via the local
       *  mobileSettingsOpen state since Settings has no global
       *  open-state on desktop (it's a view-mode there). */}
      <MobileMenuSheet
        open={menuOpen}
      ...
      {/* Friends bottom sheet — reads useFriendsSheet so any
       *  trigger anywhere in the app opens the right chrome for
       *  this breakpoint. */}
      <MobileFriendsSheet />

      {/* Settings bottom sheet — opened by the drawer's Settings
       *  row. State stays local; desktop's Settings is a view-mode,
       *  not a sheet. */}
      <MobileSettingsSheet
